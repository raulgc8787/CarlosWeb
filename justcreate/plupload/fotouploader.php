...
$id = $_SESSION['id'];
$targetDir = '../../pics_'.$tabla.'/'.$id;
//$idioma = $_SESSION['idioma'];

//Busco el orden maximo que tiene una foto de esta propiedad
$max_orden = mysqli_query($link,"SELECT MAX(orden) AS max FROM ".$tabla." WHERE id_elem=".$id." AND idioma='".$idiom."'");
...
if (!$chunks || $chunk == $chunks - 1) {
	...
	$c = mysqli_query($link,"SELECT * FROM config_thumbs ct LEFT JOIN config_tipothumb ctt ON (ctt.id = ct.select_tipothumb) ORDER BY ct.id ASC");
	while($con = mysqli_fetch_array($c)){
		...
		if($tipus_thumbnail == "width"){
			...
			// Source image
			$extension = substr($fileName,strlen($fileName)-3,3);
			if($extension == "gif"){
				$src = imagecreatefromgif("../../pics_".$tabla."/".$_SESSION['id']."/".$fileName);
				...
				$src = imagecreatefrompng("../../pics_".$tabla."/".$_SESSION['id']."/".$fileName);
				// Destination image with white background
				$dst = imagecreatetruecolor($width_final, $height_final);
				imagesavealpha($dst, true);
				imagefill($dst, 0, 0, imagecolorallocatealpha($dst, 255, 255, 255,127));
	...
	$inserta_foto = mysqli_query($link,"INSERT INTO `".$tabla."`(id_elem,foto,orden,idioma) VALUES('".$id."','".$fileName."','".$nuevo_orden."','".$idiom."')");
}

$_SESSION['idioma'] = $idiom;
